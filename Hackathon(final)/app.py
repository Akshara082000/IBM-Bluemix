...
def map():
      ...
      if request.method == "POST": 
          ...
          try :
               ...
               return  render_template('display.html',lang=longitude,lat=latitude,Country=Country,ERROR_MSG="No results found for this Location")
          ...
          pos_hash,neg_hash=h.hashtags()
        
          data={'Positive':positive_tweet_percent,'Negative':Negative_tweet_percent,'Tweets':No_of_tweets}
          return render_template('display.html',lang=longitude,lat=latitude,Country=Country,
          data=data,postweetslen=len(Positive_tweets),negtweetslen=len(Negative_tweets),
          words=dict_of_words,freq=dict_of_frequency,pos_hash=pos_hash,neg_hash=neg_hash,ERROR_MSG="Found")
      else:
